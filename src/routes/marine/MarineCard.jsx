...
export  const MarineCard = ({ opts}) => {
  ...
  const toggleActiveProduct = (e) => {
    ...
    const properProduct = marineProducts.find((el) => el.model === clickedProductId);
...
<h4 className="price-dealer">From: $ <span id={`dp-${idNum}`}>{quarters ? getPrice() : getPrice()}</span> </h4>
      ...
      {!modal ? null : (
            <BookingMarine opts={{ rem: serviceDecider(), txt: "", reset: setModal, dPrice: document.getElementById(`dp-${idNum}`).innerHTML}} />
          )}
