...
export const CarAudioCard = ({ opts}) => {
  const {ident, heatedseats, audio, remote, model, name, includes, dealerPrice, retailPrice, dealerPriceCar, dealerPriceSuv, dealerPriceTruck, retailPriceCar, retailPriceSuv, retailPriceTruck, image, dlr, quarters, idNum } = opts;
  ...
  const toggleActiveProduct = (e) => {
    ...
    const properProduct = carAudioProducts.find((el) => el.model === clickedProductId);
  ...
  const getPrice = (e)=> {
    if(!remote && vehic==="car") return dealerPriceCar;
    if(!remote && vehic==="suv") return dealerPriceCar;
    if(!remote && vehic==="truck") return dealerPriceCar;
    ...
    if(vehic==="suv" && !ceramic && !brow && !ultimate) return dealerPriceCar;
    if(vehic==="truck" && !ceramic && !brow && !ultimate) return dealerPriceCar;

    if(vehic==="car" && ultimate) return Math.round(dealerPriceCar * 1.5).toFixed(2);
    if(vehic==="suv" && ultimate) return Math.round(dealerPriceCar * 1.5).toFixed(2);
    if(vehic==="truck" && ultimate ) return Math.round(dealerPriceCar * 1.5).toFixed(2);
